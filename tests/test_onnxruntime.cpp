...
class Network {
 public:
  Network()
      : env_(),
        session_options_(),
        session_(env_, "onnx_model/phoneme_trantision_model.onnx", session_options_),
        memory_info_(Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeCPU)),
        run_options_() {
    ;
  }

  void run(char const *mono_16kHz_16bit_wav_file, std::string const &output_hdf5_file) {
    ...
    saveFloatArrayHDF5(y, shape[1], shape[2], output_hdf5_file);
  }
...
TEST(Test_ONNXModel, test_can_work) {
  ...
  n.run("tests/wavdata/dowaNgo.wav", "tests/prediction_logprobs/dowaNgo.h5");
  n.run("tests/wavdata/tasuuketsU.wav", "tests/prediction_logprobs/tasuuketsU.h5");
  n.run("tests/wavdata/ishIkI.wav", "tests/prediction_logprobs/ishIkI.h5");
}