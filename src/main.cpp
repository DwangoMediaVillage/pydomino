...

    {
...

      {
        ...
        std::filesystem::path const input_path =
            program.present<std::string>("--input_path").value();
        if (std::filesystem::is_directory(input_path)) {
          ...
          for (std::filesystem::directory_entry const &file :
            ...
            if (!std::filesystem::is_regular_file(wav_file) ||
                (wav_file.extension() != ".wav")) {
              continue;
            }
            ...
            std::filesystem::path const txt_file =
                with_suffix(wav_file, ".txt");
            std::filesystem::path const lab_file =
                with_suffix(wav_file, ".lab", output_dir);
            std::filesystem::path const log_file =
                with_suffix(lab_file, ".log");
            ...
            std::cout << "load_wav(" << load_result << "): " << wav_data.size()
                      << std::endl;

            auto const labels = aligner.align(wav_data.data(), wav_data.size(),
                                              phonemes_index, N);
            write_lab_file(labels, lab_file);
          }
        } else if (std::filesystem::is_regular_file(input_path) &&
                   input_path.extension() == ".wav") {
          ...
          std::filesystem::path const lab_file =
              program.present<std::string>("--output_path")
                  ? std::filesystem::path(
                        program.present<std::string>("--output_path").value())
          ...
          std::vector<int> const phonemes_index =
              program.present<std::string>("--input_phoneme")
                  ? domino::Aligner::read_phonemes(
                        program.present<std::string>("--input_phoneme").value())
                  : domino::Aligner::read_phonemes(txt_file);
          ...
          // エラー処理
          throw std::runtime_error("invalid input_path: " +
                                   input_path.string());
        }
      }
    }
